...
**SQL status — DRAFTED, NOT applied.** `supabase/sql/phase3_bank_config.sql` (+ `phase3_bank_config_down.sql`) exist and are ready, but have NOT been run against the live Supabase project "arca". Per this change's established precedent (Phase 0, Phase 2), live execution requires a separate, explicit project-owner sign-off that has not yet been given for Phase 3. Everything else below (TS feature code, the serverless endpoint, the public vista entry, and their tests) has no live-DB dependency and is implemented and verified.

- [ ] 3.1 `supabase/sql/phase3_bank_config.sql`: `configuracion_bancaria` DDL (singleton `check (id = 1)`), `is_admin()` policies, seed row; apply. **Drafted, ready, NOT applied — awaiting live sign-off.** One deviation from the literal design.md RLS table: since `CREATE POLICY`'s `FOR` clause accepts only one of `ALL|SELECT|INSERT|UPDATE|DELETE` (never a comma list), and design.md gives SELECT and every write the identical `is_admin()` predicate for this table, the script uses a single `admins_all_configuracion_bancaria` `FOR ALL` policy — mirroring phase2_rls.sql's `admins_all_*` policies on the 4 original tables, not two separate SELECT/write policies.
...
- [ ] 3.5 Set unprefixed `SUPABASE_URL`/`SUPABASE_SERVICE_ROLE_KEY` in Vercel (server-only); add a `postbuild` script grepping `dist/` for `service_role`, non-zero exit on match. **Postbuild script done and functionally verified** (`scripts/check-no-service-role.mjs`, wired via `package.json`'s `postbuild`): confirmed it exits non-zero when a `service_role` string is deliberately injected into `dist/`, and exits 0 on a clean build — including a build with the actual `VITE_SUPABASE_URL`/`VITE_SUPABASE_ANON_KEY` env vars set (client.ts's own guard throws unconditionally and Rollup tree-shakes the entire app as dead code when those vars are absent, as they are in this sandbox — a dummy-value build was needed to verify the *real* bundle, not a stub). **Vercel dashboard step NOT done** — no Vercel credentials/CLI access from this environment; the human/owner must set `SUPABASE_URL` (unprefixed) and `SUPABASE_SERVICE_ROLE_KEY` (unprefixed, **Secret** type, server-only) in the Vercel project settings before `api/debt-view.ts` can return real data live.
- [ ] 3.6 Manual E2E on preview deploy: `/vista/` with no session shows debt + bank and nothing else; `<script>alert(1)</script>` nickname renders as text everywhere; confirm both `/vista/` and `/api/debt-view` resolve (D2 apply-time check). **Not done** — requires a live Vercel preview deploy plus the live `configuracion_bancaria` table (3.1, not yet applied) and the Vercel env vars (3.5, not yet set). Deferred until both land.
