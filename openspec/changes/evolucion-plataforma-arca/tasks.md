...
- [ ] 3.6 Manual E2E on preview deploy: `/vista/` with no session shows debt + bank and nothing else; `<script>alert(1)</script>` nickname renders as text everywhere; confirm both `/vista/` and `/api/debt-view` resolve (D2 apply-time check). **Not done** — the live `configuracion_bancaria` table (3.1) is now applied, but this still needs a live Vercel preview deploy and the Vercel env vars (3.5, not yet set). Deferred until 3.5 lands.
