...
- [ ] 2.1 Draft `supabase/sql/portal_miembros_token.sql` — additive `token_hash text check (token_hash is null or token_hash ~ '^[0-9a-f]{64}$')` + `token_generado_en timestamptz` on `miembros`, plus partial unique index `miembros_token_hash_key on miembros(token_hash) where token_hash is not null`. **DRAFT ONLY — do not apply.**
- [ ] 2.2 Draft `supabase/sql/portal_miembros_token_down.sql` — verbatim `drop column token_hash` (drops the partial index with it) + `drop column token_generado_en`. Write before any live execution.
