...
**Status:** code, the four live migrations (phases 5–8), and the artifact passes are DONE. The duplicate "Registrar Egreso" form is REMOVED. The change is merged to `main` and LIVE in production; it is NOT archived. Verification items confirmed against the live site are checked with a note stating what was confirmed and what was not. ALL TASKS ARE NOW COMPLETE. V.3 (negative-balance rendering) was exercised deliberately against production on 2026-09-15 in a real browser, on all three surfaces, and then reverted — see V.3 below for the evidence and the reversal.
...
- [x] V.3 Confirm negative `cajaCents` renders red + unblocked on admin AND public surfaces. **EXERCISED DELIBERATELY 2026-09-15 in a real browser, then reverted.** Because the Arca is positive in normal operation, the negative path is unreachable without a temporary opening, so `configuracion_caja.monto_apertura` was temporarily set to a synthetic value chosen so the derived Arca was exactly −1,000.00. All THREE surfaces were confirmed live with that negative: the admin dashboard "Arca (disponible)" KPI, the admin Arca module breakdown, and the public `/vista/` with no session. Each rendered the figure in red (`text-red-600`) and unblocked — never hidden, never clamped to zero, no error and no "insufficient balance" guard. The admin module also showed the two outflow cards separate under "Salidas del arca" and "Por cobrar" rendered as context, NOT summed into the balance: the displayed figures satisfied `apertura + pagos − apoyos − egresos` exactly. **Reverted the same session:** `monto_apertura` restored to its original value and confirmed by read-back (derived Arca back to its correct positive value). Only that one column was ever written, so `updated_by`/`updated_at` keep their original values — zero residual trace. Caveat: the public endpoint's edge cache (`max-age=60` + `stale-while-revalidate=300`) may serve the negative briefly after the revert — documented staleness, not a defect.
