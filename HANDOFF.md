...
**Phase 2 — DONE (SQL applied live + TypeScript), NOT pushed.** `app_admins` table, `is_admin()`/`is_superadmin()` helpers, the 5-superadmin-only RLS overhaul, the AAL2 restrictive layer, and the last-superadmin guard trigger are all live on the shared Supabase project — applied with explicit project-owner sign-off in-session (same pattern as Phase 0), each step verified by read-back before the next ran. `fors@gmail.com` is now the sole account with any access to `miembros`/`cargos`/`registro_apoyos`/`registro_pagos` — **`alexis@gmail.com` and every other account lost access the moment this ran**; re-granting needs a manual `insert into app_admins` (no UI path yet, see below). `src/features/auth/mfa.ts` and `src/features/admin/{index,repo,errors}.ts` are written and unit-tested (140/140) but **not wired into `main.ts`/`index.html` navigation yet** — no admin panel is reachable in the live UI, and no forced-MFA-enrollment screen exists yet either, even though the DB now requires AAL2 for `app_admins` writes. Commit `f16053e` (SQL+TS drafted) + the 5 SQL file status-comment updates + `tasks.md` update (not pushed yet as of this note — see Git below).

**Day-to-day impact for `fors@gmail.com`**: none — `is_admin()` (used by the miembros/cargos/apoyos/pagos policies) has no AAL requirement, so normal dues/payment tracking works exactly as before at aal1 login. Only writes to `app_admins` itself (managing other admins) require AAL2, which is where the missing enrollment UI below actually bites.

**Follow-ups a new session must handle**:
- **Urgent-ish**: wire the admin panel and MFA-enrollment flow into the actual app shell (`main.ts`, nav, routing) — the backend now enforces AAL2 for admin writes but there's no UI path for `fors@gmail.com` to actually enroll TOTP or reach the admin list yet. Until this lands, the superadmin can log in (aal1) but has no way to self-serve MFA enrollment or re-grant `alexis@gmail.com`'s access through the UI.
- Task 2.6 (manual RLS test as real anon/aal1/aal2 sessions) is not done — the Management API bypasses RLS, so this needs a human testing through the actual live app.
- Task 2.8's open question: there's no browser-safe way to look up a user by email to add them as an admin (self-signup is closed) — `addAdmin` takes a raw `user_id` UUID the operator must already have. Needs a decision in a later phase.
...
**Git**: everything through Phase 2's SQL-drafting + rollback script (`e76a857` PR2, `b10fbc2` PR3, `c798fdf` docs, `f16053e` PR4 SQL+TS, `8559f0b` rollback script) is committed AND pushed to `origin/main`. The Phase-2-applied-live status updates (tasks.md rewrite + 5 SQL file header changes) made after live execution are pending a commit — see Next Steps. The default `gh`/git identity on this machine is `consultores-orion`, which does NOT have write access to this repo — all pushes used a one-off classic PAT for `skyclick1939` (stored in Engram, topic `config/github-pat-skyclick1939-control-admon-poncitlan`), passed via an inline authenticated URL rather than `gh auth login`. Use the same pattern for future pushes.
